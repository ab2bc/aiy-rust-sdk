...
use aiy_sdk_types::Address;

use crate::field::FieldMaskUtil;
use crate::proto::aiy::rpc::v2beta2::simulate_transaction_request::TransactionChecks;
use crate::proto::aiy::rpc::v2beta2::Argument;
use crate::proto::aiy::rpc::v2beta2::GetObjectRequest;
use crate::proto::aiy::rpc::v2beta2::Input;
use crate::proto::aiy::rpc::v2beta2::ListOwnedObjectsRequest;
use crate::proto::aiy::rpc::v2beta2::MoveCall;
use crate::proto::aiy::rpc::v2beta2::Object;
use crate::proto::aiy::rpc::v2beta2::ProgrammableTransaction;
use crate::proto::aiy::rpc::v2beta2::SimulateTransactionRequest;
use crate::proto::aiy::rpc::v2beta2::Transaction;
...
pub struct DelegatedStake {
    /// ObjectId of this StakedAiy delegation.
    pub staked_aiy_id: Address,
    /// Validator's Address.
...
impl Client {
    pub async fn get_delegated_stake(&mut self, staked_aiy_id: &Address) -> Result<DelegatedStake> {
        let maybe_staked_aiy = self
            .ledger_client()
            .get_object(GetObjectRequest {
                object_id: Some(staked_aiy_id.to_string()),
                version: None,
        ...
        let mut stakes = self
            .try_create_delegated_stake_info(&[maybe_staked_aiy])
            .await?;
        Ok(stakes.remove(0))
    }

    pub async fn list_delegated_stake(&mut self, address: &Address) -> Result<Vec<DelegatedStake>> {
        const STAKED_AIY_TYPE: &str = "0x3::staking_pool::StakedAiy";
        ...
        let mut list_request = ListOwnedObjectsRequest {
            ...
            page_token: None,
            read_mask: Some(FieldMask::from_str("contents")),
            object_type: Some(STAKED_AIY_TYPE.to_owned()),
        };
        ...
        loop {
            ...
            // with the fetched StakedAiy objects, attempt to calculate the rewards and create a
    ...
    async fn try_create_delegated_stake_info(
        &mut self,
        maybe_staked_aiy: &[Object],
    ) -> Result<Vec<DelegatedStake>> {
        let staked_aiys = maybe_staked_aiy
            .iter()
            .map(|o| {
                o.contents
                    .clone() // Avoid clone probably with better getters
                    .unwrap_or_default()
                    .deserialize::<StakedAiy>()
                    .map_err(Into::into)
                    .map_err(tonic::Status::from_error)
            })
            .collect::<Result<Vec<StakedAiy>>>()?;

        let ids = staked_aiys.iter().map(|s| s.id).collect::<Vec<_>>();
        let pool_ids = staked_aiys.iter().map(|s| s.pool_id).collect::<Vec<_>>();
        ...
        Ok(staked_aiys
            ...
            .map(
                |((staked_aiy, (_id, rewards)), (_pool_id, validator_address))| DelegatedStake {
                    staked_aiy_id: staked_aiy.id,
                    validator_address,
                    staking_pool: staked_aiy.pool_id,
                    activation_epoch: staked_aiy.stake_activation_epoch,
                    principal: staked_aiy.principal,
                    rewards,
                },
            )
            .collect())
    ...
    async fn calculate_rewards(
        ...
        for id in staked_aiy_ids {
            let staked_aiy = Argument::new_input(ptb.inputs.len() as u16);
            ...
            ptb.commands.push(
                MoveCall {
                    ...
                    module: Some("aiy_system".to_owned()),
                    function: Some("calculate_rewards".to_owned()),
                    type_arguments: vec![],
                    arguments: vec![system_object, staked_aiy],
                }
                .into(),
            );
        }
        ...
        if staked_aiy_ids.len() != resp.outputs.len() {
            return Err(tonic::Status::from_error(
                "missing transaction command outputs".into(),
            ));
        }

        let mut rewards = Vec::with_capacity(staked_aiy_ids.len());

        for (id, output) in staked_aiy_ids.iter().zip(resp.outputs) {
