...
use aiy_sdk_types::PersonalMessage;
use aiy_sdk_types::Transaction;
...
/// Interface for signing user transactions and messages in Aiy
///
/// # Note
///
/// There is a blanket implementation of `AiySigner` for all `T` where `T:
...
impl<T: Signer<UserSignature>> AiySigner for T {
    fn sign_transaction(&self, transaction: &Transaction) -> Result<UserSignature, SignatureError> {
...
/// Interface for verifying user transactions and messages in Aiy
///
/// # Note
///
/// There is a blanket implementation of `AiyVerifier` for all `T` where `T:
...
impl<T: Verifier<UserSignature>> AiyVerifier for T {
    fn verify_transaction(
        &self,
        transaction: &Transaction,
        signature: &UserSignature,
